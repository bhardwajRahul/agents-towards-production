...
Sections 1 to 4 run anywhere, Colab included, with no API key and no account. They are the part
that shows the cost. Sections 5 onward run the real pipeline and need the local Inngest dev server,
...
- **Node.js**, only for sections 5 onward, to run `npx inngest-cli@latest dev`
